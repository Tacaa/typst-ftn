...
#let naslov = "Текстуална и семантичка претрага YouTube транскрипата"
#let autor = "Татјана Гавриловић"

// На енглеском
#let naslov_eng = "Textual and semantic search of YouTube transcripts"
...
#let zadatak = [
     Реализовати систем за текстуалну претрагу транскрипата подкаста на српском језику објављених на платформи YouTube. Потребно је прикупити корпус епизода са већег броја канала, аутоматски га транскрибовати применом модела за препознавање говора и добијене транскрипте, са временским ознакама сегмената, сместити у релациону базу података. Над корпусом треба реализовати лексичку претрагу засновану на инвертованом индексу и BM25 рангирању, уз анализатор прилагођен српском језику, и семантичку претрагу засновану на вишејезичном embedding моделу и векторском индексу, а њихове резултате обједињавати методом реципрочне фузије рангова (RRF) уз очување грануларности која омогућава навигацију до релевантног тренутка у епизоди. Систем реализовати као контејнеризовану веб апликацију са REST сервисом и корисничким интерфејсом. Практичну проверу извршити упоредном евалуацијом лексичког, семантичког и хибридног приступа над ручно означеним скупом упита, применом стандардних мера квалитета рангирања (_`Recall@k`_, _`MRR@k`_, _`nDCG@k`_).
]
